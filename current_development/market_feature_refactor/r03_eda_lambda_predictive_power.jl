...
n_samples = min(100, nrow(valid_matches))
...
for match_row in eachrow(test_matches)
    ...
    h_goals = hasproperty(match_row, :home_score) ? match_row.home_score : match_row.home_goals
    a_goals = hasproperty(match_row, :away_score) ? match_row.away_score : match_row.away_goals
    ...
    # Rebuild the probability matrices using the extracted parameters
    ...
    # Calculate the Log-Likelihood of the ACTUAL observed score
...
println("Frank Copula:    ", round(sum_ll_fc, digits=2), " (Diff vs DP: ", round(sum_ll_fc - sum_ll_dp, digits=2), ")")

# Let's see which model was "closest" to reality most often
wins_dp = sum((results.dp_loglike .> results.dc_loglike) .& (results.dp_loglike .> results.fc_loglike))
wins_dc = sum((results.dc_loglike .> results.dp_loglike) .& (results.dc_loglike .> results.fc_loglike))
wins_fc = sum((results.fc_loglike .> results.dp_loglike) .& (results.fc_loglike .> results.dc_loglike))
