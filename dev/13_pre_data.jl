...
is_corrupt(ratio) = abs(ratio) > 1.0
...
df_analysis_1 = generate_kelly_analysis_df(model, predict_config, all_oos_results, ds)


# 2. Inspect a specific market type (e.g., Home Wins)
home_only = filter(row -> row.market == :under_25 , df_analysis)
home_only_1 = filter(row -> row.market == :over_25 , df_analysis_1)
