...
using Statistics
using Distributions
using Turing # Required for NegativeBinomial PDF and logistic function
...
1.  PARAMETER EXTRACTION
--------------------------------------------------------------------------------
This is the specialized implementation for the dynamic AR1 Negative Binomial
model. It reconstructs the full time-series of all four dynamic parameters.
=#
...
function BayesianFootball.extract_posterior_samples(
    ...
    # --- Extract AR(1) hyper-parameters ---
    ρ_attack = vec(Array(chain[:ρ_attack]))
    ρ_defense = vec(Array(chain[:ρ_defense]))
    ρ_home = vec(Array(chain[:ρ_home])) # <-- NEW
    ρ_phi = vec(Array(chain[:ρ_phi]))   # <-- NEW
    ...
    σ_home = exp.(vec(Array(chain[:μ_log_σ_home]))) # <-- NEW
    σ_phi = exp.(vec(Array(chain[:μ_log_σ_phi])))   # <-- NEW

    # --- Reconstruct the full raw state-space trajectories ---
    # Initial states
    initial_α_z = BayesianFootball.extract_samples(chain, "initial_α_z")
    initial_β_z = BayesianFootball.extract_samples(chain, "initial_β_z")
    initial_home_z = BayesianFootball.extract_samples(chain, "initial_home_z") # <-- NEW
    initial_phi_z = vec(Array(chain[:initial_phi_z])) # <-- NEW

    # Innovations
    z_α_mat_reshaped = reshape(z_α_flat', n_teams, n_rounds, n_samples)
    z_β_flat = BayesianFootball.extract_samples(chain, "z_β")
    z_β_mat_reshaped = reshape(z_β_flat', n_teams, n_rounds, n_samples)
    z_home_flat = BayesianFootball.extract_samples(chain, "z_home") # <-- NEW
    z_home_mat_reshaped = reshape(z_home_flat', n_leagues, n_rounds, n_samples) # <-- NEW
    z_phi = BayesianFootball.extract_samples(chain, "z_phi") # <-- NEW

    # State containers
    log_α_raw = Array{Float64, 3}(undef, n_samples, n_teams, n_rounds)
    log_β_raw = Array{Float64, 3}(undef, n_samples, n_teams, n_rounds)
    log_home_adv_raw = Array{Float64, 3}(undef, n_samples, n_leagues, n_rounds) # <-- NEW
    log_phi_raw = Array{Float64, 2}(undef, n_samples, n_rounds) # <-- NEW

    # Evolution loop
    for s in 1:n_samples
        log_α_raw_t0 = initial_α_z[s, :] .* sqrt(0.5)
        log_β_raw_t0 = initial_β_z[s, :] .* sqrt(0.5)
        log_home_adv_raw_t0 = log(1.3) .+ initial_home_z[s, :] .* sqrt(0.1) # <-- NEW
        log_phi_raw_t0 = log(10.0) + initial_phi_z[s] * sqrt(0.1) # <-- NEW

        for t in 1:n_rounds
            if t == 1
                log_α_raw[s, :, 1] = log_α_raw_t0 .+ z_α_mat_reshaped[:, 1, s] .* σ_attack[s, :]
                log_β_raw[s, :, 1] = log_β_raw_t0 .+ z_β_mat_reshaped[:, 1, s] .* σ_defense[s, :]
                log_home_adv_raw[s, :, 1] = log_home_adv_raw_t0 .+ z_home_mat_reshaped[:, 1, s] .* σ_home[s] # <-- NEW
                log_phi_raw[s, 1] = log_phi_raw_t0 + z_phi[s, 1] * σ_phi[s] # <-- NEW
            else
                log_α_raw[s, :, t] = ρ_attack[s] * log_α_raw[s, :, t-1] .+ z_α_mat_reshaped[:, t, s] .* σ_attack[s, :]
                log_β_raw[s, :, t] = ρ_defense[s] * log_β_raw[s, :, t-1] .+ z_β_mat_reshaped[:, t, s] .* σ_defense[s, :]
                log_home_adv_raw[s, :, t] = ρ_home[s] * log_home_adv_raw[s, :, t-1] .+ z_home_mat_reshaped[:, t, s] .* σ_home[s] # <-- NEW
                log_phi_raw[s, t] = ρ_phi[s] * log_phi_raw[s, t-1] + z_phi[s, t] * σ_phi[s] # <-- NEW
            end
        end
    end

    # Pre-calculate centered parameters for efficiency
    log_α_centered = similar(log_α_raw)
    log_β_centered = similar(log_β_raw)
    for s in 1:n_samples, t in 1:n_rounds
        log_α_centered[s, :, t] = log_α_raw[s, :, t] .- mean(log_α_raw[s, :, t])
    ...
    return (
        n_rounds = n_rounds,
        ρ_attack = ρ_attack, ρ_defense = ρ_defense, ρ_home = ρ_home, ρ_phi = ρ_phi,
        σ_attack = σ_attack, σ_defense = σ_defense, σ_home = σ_home, σ_phi = σ_phi,
        log_α_raw = log_α_raw, log_β_raw = log_β_raw, 
        log_home_adv_raw = log_home_adv_raw, log_phi_raw = log_phi_raw,
...
#=
--------------------------------------------------------------------------------
2.  TIME-AWARE GOAL RATE & PARAMETER CALCULATION
--------------------------------------------------------------------------------
This function now handles all four dynamic parameters.
=#
...
function get_neg_bin_params(
    ...
    home_idx = features.home_team_ids[1]
    away_idx = features.away_team_ids[1]
    league_idx = features.league_ids[1] # <-- NEW
    current_round = features.global_round[1]
    ...
    if current_round <= samples.n_rounds
        ...
        log_α_t = samples.log_α_centered[i, :, current_round]
        log_β_t = samples.log_β_centered[i, :, current_round]
        log_home_adv_i = samples.log_home_adv_raw[i, league_idx, current_round] # <-- NEW
        ϕ_i = exp(samples.log_phi_raw[i, current_round]) # <-- NEW
        ...
        # Get the state from the final round of data
        last_α_raw = samples.log_α_raw[i, :, samples.n_rounds]
        last_β_raw = samples.log_β_raw[i, :, samples.n_rounds]
        last_home_adv_raw = samples.log_home_adv_raw[i, :, samples.n_rounds]
        last_phi_raw = samples.log_phi_raw[i, samples.n_rounds]
        
        # Propagate all states one step forward using their AR(1) process
        future_α_raw = samples.ρ_attack[i] .* last_α_raw .+ randn(n_teams) .* samples.σ_attack[i, :]
        future_β_raw = samples.ρ_defense[i] .* last_β_raw .+ randn(n_teams) .* samples.σ_defense[i, :]
        ...
        # Center strengths and select correct parameters for the match
        log_α_t = future_α_raw .- mean(future_α_raw)
        log_β_t = future_β_raw .- mean(future_β_raw)
    ...
    # Calculate goal rates and convert to success probabilities `p`
    log_λ_home = log_α_t[home_idx] + log_β_t[away_idx] + log_home_adv_i
    log_λ_away = log_α_t[away_idx] + log_β_t[home_idx]
...
#=
--------------------------------------------------------------------------------
3.  PREDICTION LOOP & API INTEGRATION (Largely unchanged)
--------------------------------------------------------------------------------
The functions below orchestrate the prediction but the core logic is now inside
`get_neg_bin_params`. They require minimal changes.
=#

# Helper for Negative Binomial score matrix
function _compute_xScore_neg_bin(ϕ::Number, p_home::Number, p_away::Number, max_goals::Int)
    p = zeros(max_goals + 1, max_goals + 1)
    for h in 0:max_goals, a in 0:max_goals
        p[h+1, a+1] = pdf(NegativeBinomial(ϕ, p_home), h) * pdf(NegativeBinomial(ϕ, p_away), a)
...
function predict_ar1_neg_bin_match_lines(
    ...
    # Note: We now pass mapping to the lower-level functions because it's needed
    # for dimensions in extract_posterior_samples.
    ht_predict = _predict_ar1_neg_bin_match_ht(model_def, round_chains.ht, features, posterior_samples)
    ft_predict = _predict_ar1_neg_bin_match_ft(model_def, round_chains.ft, features, posterior_samples)
...
function _predict_ar1_neg_bin_match_ft(
    ...
    n_samples = size(chain, 1) * size(chain, 3)
    # [cite_start]Initialize result containers... (identical to source file) [cite: 16, 17, 18, 19]
    λ_home = zeros(n_samples); λ_away = zeros(n_samples)
    home_win_probs = zeros(n_samples); draw_probs = zeros(n_samples); away_win_probs = zeros(n_samples)
    under_05 = zeros(n_samples); under_15 = zeros(n_samples); under_25 = zeros(n_samples); under_35 = zeros(n_samples)
    btts = zeros(n_samples)
    ...
    for i in 1:n_samples
        params = get_neg_bin_params(model_def, posterior_samples, i, features)
        λ_home[i] = params.λ_home
        λ_away[i] = params.λ_away
...
# The _predict_ar1_neg_bin_match_ht function would follow the same pattern as above.
# It is omitted for brevity but would be structured identically to _predict_ar1_neg_bin_match_ft.
function _predict_ar1_neg_bin_match_ht(
    model_def::AR1NegativeBinomialHAModel,
    chain::Chains,
    features::NamedTuple,
    ...
    n_samples = size(chain, 1) * size(chain, 3)
    # [cite_start]Initialize result containers... (identical to source file) [cite: 23, 24, 25, 26]
    λ_home = zeros(n_samples); λ_away = zeros(n_samples)
    home_win_probs = zeros(n_samples); draw_probs = zeros(n_samples); away_win_probs = zeros(n_samples)
    under_05 = zeros(n_samples); under_15 = zeros(n_samples); under_25 = zeros(n_samples)
    cs_keys = vcat([(h, a) for h in 0:2 for a in 0:2], ["any_unquoted"])
...
end # end module AR1NegBiPrediction_ha
