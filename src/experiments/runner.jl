...
function save_artifacts(path, chains, config)
    mkpath(path)
    jldsave(joinpath(path, "chains.jld2"); chains)
    # Prediction artifacts would be saved here too
...
function run_experiment(exp::Experiment, data_store::DataStore; base_save_path::String)
    ...
    # --- NEW STEP 1: Create the Global Vocabulary (G) ---
    println("Creating global vocabulary...")
    vocabulary = Features.create_vocabulary(data_store, exp.model)
    
    # --- NEW STEP 2: Save the global vocabulary ---
    jldsave(joinpath(exp_path, "vocabulary.jld2"); vocabulary)
    println("✅ Global Vocabulary created and saved.")
    
    # --- STEP 3: Run the splitter, passing the vocabulary into it ---
    _run_splitter(exp, exp.splitter, data_store.matches, vocabulary, exp_path)
end

# --- Runner for StaticSplit ---

function _run_splitter(exp::Experiment, splitter::StaticSplit, all_matches_df::DataFrame, vocabulary::Vocabulary, exp_path::String)
    println("Running as a single static split...")
    
    train_df = filter(row -> row.season in splitter.train_seasons, all_matches_df)
    
    _run_pipeline_step(exp, train_df, vocabulary, exp_path, split_name="static_run")
end

# --- Runner for ExpandingWindowCV ---

function _run_splitter(exp::Experiment, splitter::ExpandingWindowCV, all_matches_df::DataFrame, vocabulary::Vocabulary, exp_path::String)
    ...
    @showprogress "Running CV Splits..." for (split_idx, (train_view, round_info)) in enumerate(cv_iterator)
        ...
        _run_pipeline_step(
            exp, 
            DataFrame(train_view), # This is D_i
            vocabulary,            # This is G
            split_save_path,
...
# --- The Core Pipeline (f_i -> g -> h) ---

function _run_pipeline_step(
    exp::Experiment, 
    train_df::DataFrame,      # This is D_i
    vocabulary::Vocabulary,   # This is G
    save_path::String; 
    split_name::String
)
    
    # --- 1. Morphism f_i: (D_i, G, M) -> F_i ---
    features = Features.create_features(train_df, vocabulary, exp.model)
    ...
    # --- 4. Prediction Morphism h would go here ---
    
    # --- 5. Save Artifacts for this split ---
    save_artifacts(save_path, chains_params, exp)
