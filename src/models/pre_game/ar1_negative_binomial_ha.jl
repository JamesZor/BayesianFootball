# workspace/basic_state_space/models/ar1_negative_binomial_ha.jl
...
struct AR1NegativeBinomialHAModel <: AbstractNegativeBinomialSSM end
...
function BayesianFootball.extract_posterior_samples(
    ...
    ρ_attack=vec(Array(chain[:ρ_attack])); ρ_defense=vec(Array(chain[:ρ_defense])); ρ_home=vec(Array(chain[:ρ_home])); ρ_phi=vec(Array(chain[:ρ_phi]))
    ...
    σ_home = exp.(vec(Array(chain[:μ_log_σ_home]))); σ_phi = exp.(vec(Array(chain[:μ_log_σ_phi])))
    ...
    initial_home_z = BayesianFootball.extract_samples(chain, "initial_home_z"); initial_phi_z = vec(Array(chain[:initial_phi_z]))
...
function get_neg_bin_params(
    ...
    if current_round <= samples.n_rounds
        ...
        log_home_adv_i = future_home_adv_raw[league_idx]; ϕ_i = exp(future_phi_raw)
    end
