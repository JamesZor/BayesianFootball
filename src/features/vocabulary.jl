...
function adapt_vocabulary(global_vocab::Vocabulary, df::AbstractDataFrame)
    ...
    if hasproperty(df, :home_team); union!(present_teams, df.home_team); end
    if hasproperty(df, :away_team); union!(present_teams, df.away_team); end
    
    # 2. Sort for deterministic ordering (Crucial for reproducibility)
    sorted_teams = sort(collect(present_teams))
    ...
    # 4. Return new Vocabulary
    # We copy other keys (like league_map) if needed, or just rebuild the basics
    return Vocabulary(Dict(
        :team_map => new_map,
        :n_teams => length(sorted_teams)
    ))
