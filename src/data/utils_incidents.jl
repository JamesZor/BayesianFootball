...
function _return_nothing_period(period::AbstractString)
  if period == "ht"
    cs_ht = Dict{Union{Tuple{Int,Int}, String}, MaybeBool}(
    ...
    return MatchHTResults(
      nothing, nothing, nothing, cs_ht,
      nothing, nothing, nothing,
    )
  else  # ft
    cs_ft = Dict{Union{Tuple{Int,Int}, String}, MaybeBool}(
...
function process_period_data_ht(score_row)
  ...
  correct_score = Dict{Union{Tuple{Int,Int}, String}, MaybeBool}(
...
function process_period_data_ft(score_row)
  if isnothing(score_row)
...
function get_match_results(data_store::DataStore, match_id::Int)
    ...
    ht_results = if isnothing(ht_row_idx)
        ...
        process_period_data_ht(match_incidents[ht_row_idx, :])
    end
    
    ft_results = if isnothing(ft_row_idx)
        _return_nothing_period("ft")
    else
        process_period_data_ft(match_incidents[ft_row_idx, :])
    end
    
    return MatchLinesResults(ht_results, ft_results)
end
...
function process_matches_results(data_store::DataStore, target_matches::DataFrame)
    ...
    thread_results = Vector{Dict{Int64, MatchLinesResults}}(undef, n_threads)
    
    @threads for tid in 1:n_threads
        # Each thread creates its own dictionary
        local_dict = Dict{Int64, MatchLinesResults}()
